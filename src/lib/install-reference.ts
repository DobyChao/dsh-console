/**
 * README install-command parser, adapted from dsh-plugins-store
 * (MIT, https://github.com/ZASENJC/dsh-plugins-store).
 * Profile checks use the current launcher instance profile, not a hardcoded `web`.
 */
...
const INSTALL_HEADING =
  /(?:install(?:ation)?|setup|get(?:ting)? started|quick\s*start|安装|安装方法|安装方式|快速开始)/i;
const GITHUB_SPECIFIER =
  /^github:([A-Za-z0-9](?:[A-Za-z0-9_.-]{0,99})\/[A-Za-z0-9](?:[A-Za-z0-9_.-]{0,99}))(?:#([A-Za-z0-9][A-Za-z0-9_.:-]{0,127}))?$/;
const NPM_PACKAGE =
  /^(?:@[A-Za-z0-9](?:[A-Za-z0-9._-]{0,99})\/)?[A-Za-z0-9](?:[A-Za-z0-9._-]{0,99})(?:@[A-Za-z0-9^~<>=*+._-][A-Za-z0-9^~<>=*+._-]{0,127})?$/;
const SOURCE_SHA = /^[a-f0-9]{40}$/i;
const UNSAFE_COMMAND = /(?:&&|\|\||[|;&<>`$\\])/u;
const PACKAGE_MANAGER_FLAG =
  /^(?:-D|-E|-g|--global|--save|--save-dev|--save-exact|--exact|--frozen-lockfile|--workspace)$/;
...
function parseDshCommand(command: string, heading: string | null, currentProfile: string): InstallCandidate | null {
  const match = /^dsh\s+plugin(?:\s+--profile\s+([A-Za-z0-9_-]+))?\s+add\s+(\S+)$/u.exec(command);
  if (!match) return null;

  const profile = match[1] ?? currentProfile;
  ...
  return {
    ...
    command,
    args: ["add", specifier],
    executable: profile === currentProfile && (parsed.source === "github" || parsed.source === "npm"),
    profile,
...
function parsePackageManagerCommand(
  ...
  if (
    specifiers.length !== 1 ||
    tokens.some((token) => token !== specifiers[0] && !PACKAGE_MANAGER_FLAG.test(token))
  ) {
    return null;
  }
...
export function extractInstallReference(readme: string, currentProfile: string): InstallReference {
  ...
  for (const block of collectCodeBlocks(readme)) {
    for (const line of block.lines) {
      ...
      const candidate =
        parseDshCommand(command, block.heading, currentProfile) ??
        parsePackageManagerCommand(command, block.heading, currentProfile);
  ...
  if (candidates.length > 1) {
    const executableCandidates = candidates.filter(({ executable }) => executable);
    if (executableCandidates.length === 1) {
      return { status: "recognized", candidate: executableCandidates[0], candidates };
    }
...
export function resolveCatalogInstallReference(
  ...
  repository: {
    fullName: string;
    validation?: { overall?: string; sourceSha?: string | null };
  },
  currentProfile: string,
): InstallReference {
  ...
  if (candidate.source === "github") {
    ...
    if (
      ...
      candidate.profile !== currentProfile
    ) {
      candidate.executable = false;
    } else if (verified) {
      const pinnedSpecifier = `github:${repository.fullName}#${sourceSha.toLowerCase()}`;
      candidate.command = `dsh plugin --profile ${currentProfile} add ${pinnedSpecifier}`;
      candidate.args = ["add", pinnedSpecifier];
      candidate.target = repository.fullName;
    }
  } else if (candidate.source === "npm") {
    if (!securityReview && candidate.evidence.pattern === "package-manager-add") {
      candidate.args = ["add", candidate.target];
      candidate.executable = candidate.profile === currentProfile;
...
export function isPinnedSha(spec: string): boolean {
  return /#[a-f0-9]{40}$/i.test(spec);
}
